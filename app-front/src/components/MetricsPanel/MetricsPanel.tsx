...
interface Props {
  metrics: EvaluationSummaryResponse; // 👈 ahora recibe directamente la respuesta del hook
  compact?: boolean;
  className?: string;
}
...
export const MetricsPanel: React.FC<Props> = ({ metrics, compact = false, className }) => {
  ...
  return (
    <section
      ...
      aria-labelledby={`metrics-${evaluationId}`}
    >
      <header className={styles.header}>
        <h4 id={`metrics-${evaluationId}`} className={styles.title}>
          {evaluationName}
        </h4>
